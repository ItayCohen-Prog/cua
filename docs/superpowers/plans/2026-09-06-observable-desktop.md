...
Research basis: Hyprland 0.56.2 local setprop help and https://wiki.hypr.land/Configuring/Basics/Window-Rules/ accessed 2026-09-06 support address-specific focus overrides. https://wayland.freedesktop.org/docs/book/Xwayland.html confirms rootful separation but host input still enters through Wayland. Merely removing the visible-workspace guard is insufficient: verify private input suppression locally. A broker is necessary because bubblewrap die-with-parent makes transferring ownership after MCP exit unreliable. No additional system packages.

- [ ] Add private XI2 device control in input_gate.py; never modify master/XTEST devices or a host display. Verify enabled state after changes, retain XTEST delivery, and reject unclassified enabled slave devices. Test agent/user transitions and private device isolation.
- [ ] Update Session validation and modes in background.py. Visibility is observation, not contention. User mode permits captures and blocks all mutations. Disable human input before resuming agent mode. Set only the owned window's no_focus override and never focus or switch it.
- [ ] Add supervisor.py with serialized local requests, controller identity, reconnect, review-on-disconnect/idle, explicit disposable cleanup, last-app-close cleanup, and status/list/stop CLI. Keep PID stamp and guardian cleanup for actual supervisor/worker failures. Add focused lifecycle tests including real process death.
- [ ] Expose finish, control, sessions and attach MCP tools. Update installer and existing background-desktop/Resolve guidance, replacing unconditional stop advice. Record behavior and verification in README and VALIDATION.
- [ ] Run unit and native Qt integration tests, verify host focus/cursor isolation and observing mode without workspace switching, verify reconnect/handoff/cleanup, install the tested Python files, then open the saved Resolve project for review if its private runtime can launch. Remove temporary test artifacts after verification.
